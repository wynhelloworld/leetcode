 ...
 *                dp[i][j] = -|- 当挑选1个n[i]时，如果j - v[i] >= 0，dp[i][j - v[i]] + w[i];
 *                            |- 当挑选2个n[i]时，如果j - 2 * v[i] >= 0，dp[i][j - 2 * v[i]] + 2 * w[i];
 *                            |- 当挑选3个n[i]时，如果j - 3 * v[i] >= 0，dp[i][j - 3 * v[i]] + 3 * w[i];
 *                            |- . . .
 ...
 *                            |- 当不挑选n[i]时，dp[i - 1][j];
 *                dp[i][j] = -|- 当挑选1个n[i]时，如果j >= v[i] && dp[i][j - v[i]] != -1，dp[i][j - v[i]] + w[i];
 *                            |- 当挑选2个n[i]时，如果j >= 2 * v[i] && dp[i][j - 2 * v[i]] != -1，dp[i][j - 2 * v[i]] + w[i];
 *                            |- 当挑选3个n[i]时，如果j >= 3 * v[i] && dp[i][j - 3 * v[i]] != -1，dp[i][j - 3 * v[i]] + w[i];
 *                            |- . . .
 *                即，状态转移方程为：{
 *                                     dp[i][j] = dp[i - 1][j];
